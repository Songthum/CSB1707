...
function ChairmanScoreCSB01() {
    ...
    const fetchProjects = async () => {
        try {
            ...
            const filteredProjects = examResponse.data.filter(result => result.Er_CSB01_status === 'ผ่าน');
            ...
            if (selectedProject) {
                try {
                    ...
                    if (projectExamResult) {
                        setData([{ id: 1, name: 'คะแนนรวม', fullscores: '90', score: projectExamResult.Er_CSB01 }]);
                    } else {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const updatedEr_CSB01 = data.find(item => item.name === 'คะแนนรวม')?.score;
                        ...
                        <Grid item>
                            <Grid container direction="column" spacing={1}>
                                <Box fontSize='18px' sx={{ marginTop: 5 }}>
                                    <h1>แบบประเมินโครงงานพิเศษ (สอบหัวข้อ)</h1>
