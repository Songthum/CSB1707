...
function InputScoreCSB01() {
  ...
  useEffect(() => {
    console.log('Selected project in state:', selectedProject);
  }, [selectedProject]);
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        const completedProjectNames = new Set(examResultsResponse.data
          .filter(result => result.Er_CSB01)
          .map(result => result.Er_Pname)
        );

        const filteredProjects = projectResponse.data
          .filter(project => !completedProjectNames.has(project.P_name));

        console.log('Fetched projects:', filteredProjects); // ตรวจสอบข้อมูลของ projects ที่ดึงมา
  ...
  const handleLinkClick = (index) => {
    ...
    console.log('Selected project:', project); // ตรวจสอบค่าที่เลือก
  ...
  const handleSavePopup = async (e) => {
    if (allChecked) {
      e.preventDefault();
      console.log('Selected project before saving:', selectedProject); // ดีบักค่า selectedProject
      if (!selectedProject || !selectedProject.P_id) {
        console.error('No project selected or Project ID is undefined.');
        alert('เกิดข้อผิดพลาดในการเลือกโครงการ กรุณาลองใหม่อีกครั้ง.');
        return;
      }
      try {
        await saveScore(); // บันทึกคะแนน
        setProjects(prevProjects => prevProjects.filter(project => project.P_id !== selectedProject.P_id));
        resetCheckboxData(); // รีเซ็ตข้อมูลฟอร์ม
  ...
  const saveScore = async () => {
    ...
    const erCSB01 = lastScore === 'ผ่านการประเมินหัวข้อ' ? 'ผ่านการประเมินหัวข้อ' : 'ไม่ผ่านการประเมินหัวข้อ';
    const erCSB01Status = lastScore === 'ผ่านการประเมินหัวข้อ' ? 'ผ่าน' : 'ไม่ผ่าน';

    try {
      const response = await axios.put(`http://localhost:9999/Exam_results/${selectedProject.P_id}`, {
        Er_Pname: erName,
        Er_CSB01: erCSB01,
        Er_CSB01_status: erCSB01Status
      });

      if (response.status === 200) {
        console.log('Score updated successfully!');
        setOpenDialog(true); // เปิด dialog ประสบความสำเร็จเมื่ออัปเดตสำเร็จ
      } else {
        console.error('Failed to update score, status code:', response.status);
      }
    } catch (error) {
